...
plot_ts <- function(file, file_conn, option = 0, ...) {
       	if(option == 1) {
	...
	plot(x[1:n], y[1:n], pch=19, ann=FALSE, cex.axis=2, ...);
	mtext(side = 1, text = "X / arb. unit", line = 3, cex=2);
	mtext(side = 2, text = "Y / arb. unit", line = 3, cex = 2);
	if(option == 1) {
		distance <- round(distance, 2);
		mtext(side = 3, text = paste("Distance:", distance), line = 1, cex=2, col = "red");
	}
